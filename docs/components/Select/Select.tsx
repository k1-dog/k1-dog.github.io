import { defineComponent, reactive, ref, PropType, Ref, onBeforeMount, onMounted, onBeforeUnmount, watch, nextTick, watchEffect, computed, ComponentInstance } from "vue"

import MikuTrans from '../Transtions'
import M9Icon from '../M9-Style/assets/_'
import { __on, __off, HangRoot, MThrottle } from '../M9-Utils'
...
import VScroller from "../VScroller/VScroller"
...
export default defineComponent({
  ...
  setup (props, ctx) {
    ...
    onBeforeMount(() => {
      const PumpkinPromiser = (async function () { return (await import('../M9-Utils/web-components/pumpkin.js')).default })()
      PumpkinPromiser.then(PumpkinBooter => { PumpkinBooter.register() })

      _initSelectWithProps(props.options)

      onFilterOptions = MThrottle(onFilterOptions.bind(null), 800, 1)
    })
    ...
    function Merge_S0_S1_IntoOptions0 (the_s1_item: MS_OPT | MS_OPTS): MS_OPTS {
      ...
      const selectOptions1 = [...state.options_1]
      // ? 选择器组件内部_全表选项_00
      const selectOptions0 = options_0
      // ? 筛选出__未被选择__的|>选项维护表<|
      .filter(opt0 => selectOptions1.findIndex(opt1 => opt1.MSVal === opt0.MSVal) < 0)

      // ! 用提取出的 本轮没参与选择逻辑的 options0- 合并 -当前最新的options1 生成全新的 options0整表选项, 覆盖老的options0 保持副本同步
      let merge_s0_s1_list = [...selectOptions0, ...selectOptions1]
      // 如果当前选择框__维护的选值类型为数组--说明是多选模式::不用处理其他选值状态
      if (!Array.isArray(the_s1_item)) {
        // 单选模式下--要处理其他选值状态
        merge_s0_s1_list = merge_s0_s1_list.map(_s01 => {
          if (_s01.MSVal !== the_s1_item.MSVal) { _s01.selected = false }
          return _s01
        })
      }

      return merge_s0_s1_list
    };
