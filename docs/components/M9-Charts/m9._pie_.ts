  ...
  __Pie__(Interactor: InteractorClassT) {
    ...
    Interactor._interactions.forEach((__interaction, index) => {
      M9Whisper.$whisper(($lastEndState, $ct) => {
        ...
        Interactor._StartAnimation_(__interaction, { startState: _startState, endState: _endState, beforeAnimating: _beforeAnimating, ctxWithCurrentState: _ctxWithCurrentState, isStop: _isStop, calcNextState: _calcNextState, M9WhisperComeTrue: $ct, done: _done })
      })
    })
    M9Whisper.$do()
  }
