  ...
  setup (props, ctx) {
    ...
    function onMultiSelect ($e: Event) {
      ...
      state.valGroup = (() => {
        ...
        if (oldValGroup) {
          const the_select_its_idx_ = oldValGroup.findIndex($old_KV => $old_KV.MSVal == NewMSK)
          if (the_select_its_idx_ < 0) {
            ...
            oldValGroup.splice(the_select_its_idx_, 1)
          }
        ...
        <HangRoot>
          ...
          >
            <div className={mselect_options_preCls} ref={($optionsPanelR) => this.optionsPanelRef = $optionsPanelR}>
              {filterable && <FilterOptsInput filteringV={filteringV} onRunFilter={onFilterOptions} onChangeFilteringV={onChangeFilteringV}/>}
...
type TMMFilterOptions = { onRunFilter: (filter_val: string | undefined) => void } & { filteringV?: string, onChangeFilteringV: (search_val: string) => void }
