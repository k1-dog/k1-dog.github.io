...
import { M9CloneDeep, T2A } from '@k1/utils'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    function reComputeFixedOffsetValue() {
      fixedOffsetValue.value = _calcFixedOffsetValue(state.M9Columns)
    }
    const fixedOffsetValue = ref({})
    provide('_fixedOffsetValue', fixedOffsetValue)
    ...
    function onRefreshM9Columns($newColumns) {
      state.M9Columns = $newColumns || [...state.M9Columns]
    }
    watch(() => state.M9Columns, ($newColumns) => {
      if (_isMounted) {
        reComputeFixedOffsetValue()
      }
    })
    ...
    return { state, tableRef, outerViewportRef, onRefreshM9Data, onRefreshM9Columns }
  },
  render() {
    const {
    ...
    // Sum of all column widths: table / vscroller width. Guarantees full horizontal
    // scroll distance inside the outer viewport so trailing columns are reachable.
    const sumColsWidth = M9Columns.reduce(($sum, $col) => $sum + ($col.width || 100), 0)

    return (
      <div style={scrollStyle} className={viewportCls} ref={($sr_: any) => this.outerViewportRef = $sr_}>
        <table
          ref={($_Tref_: any) => this.tableRef = $_Tref_}
          className={preCls}
          cellPadding={0}
          cellSpacing={0}
          style={{ borderCollapse: 'separate', borderSpacing: 0, border: 0, margin: 0, padding: 0, width: `${sumColsWidth}px` }}
        >
          {
            [
              <MTableHeader MTableHeadColumns={M9Columns} onRefreshM9Columns={onRefreshM9Columns} style={{ height: `${M9TableHHeight}px` }} />,
              <M9VScroller vsTarget={() => this.outerViewportRef} vsUnitHeight={MTRowHeight} vsStyle={{ x: sumColsWidth, h: MTScroll.h - M9TableHHeight }} data={M9Data} loadingStyle={{ position: 'sticky' }}>
                {
                  {
