...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const fixedOffsetValue: any = inject('_fixedOffsetValue')
    ...
    function onCollapseRow($rowData) {
      ...
      const callHandleMDataSource = ($m9DataState, $_storeData) => {
        const m9CloneData = M9CloneDeep($m9DataState)
        ...
        const storeRowData = $_storeData[no]
        storeRowData.isCollapse = !isCollapse
        $_storeData[no] = storeRowData

        const rowData = m9CloneData[no]
        rowData.isCollapse = !isCollapse
        m9CloneData[no] = rowData

        function commonIsCollapseControl($trusyChildSonDataLength) {
          for (let _i = no + 1; _i <= no + $trusyChildSonDataLength; _i++) {
            const storeChildSonRowData = $_storeData[_i]
            storeChildSonRowData.isHidden = $_storeData[no].isCollapse
            $_storeData[_i] = storeChildSonRowData

            const rowChildSonData = m9CloneData[_i]
            rowChildSonData.isHidden = m9CloneData[no].isCollapse
            m9CloneData[_i] = rowChildSonData
          }
        }
        // * 如果当前操作为折叠行 - 那么需要 [删除] 该节点下边 所有的子孙节点 <而且是正在展示的节点 即筛选 isHidden = false 的节点>
        // ! 注意 -
        if (storeRowData.isCollapse) {
          ...
          const hiddenLength = trusyChildSonWillHiddenData.length
          // 标记其下子孙节点 isHidden = true -> 隐藏
          commonIsCollapseControl(hiddenLength)

          m9CloneData.splice(no + 1, hiddenLength)
        }
        ...
        // ! 注意 - 上述操作有问题, 直接子级节点必须展开, 必须 isHidden = true 才是对的, 这个后续再处理吧, 现在我添加的这些属性不足以支撑这一功能设计 <主要找不到直接子级节点, 后边我估计我要处理 key值拼接了, 拼接个父级前缀标识, 通过提取前缀标识找寻是不是直接子级>
          ...
          const showLength = trusyChildSonWillShowData.length
          // 标记其下子孙节点 isHidden = false -> 显示
          commonIsCollapseControl(showLength)

          m9CloneData.splice(no + 1, 0, ...trusyChildSonWillShowData)
        }
    ...
    function __renderMTableBodyCell($rowData: any, $MColumn: MTColumnT, $MCellSequenceNo: number) {
      ...
      const MTHB_CTT = [
        ...
        ></M9Icon>
        || null,
        <span>{formatter!({ rowData: $rowData, value: $rowData[ColKey] })}</span>
      ]

      const TCellClsMixin = classNames(TCellCls, { [`${TCellCls}--fixed`]: !!fixed, [`${TCellCls}--fixed--${fixed}`]: !!fixed })

      const style = _calcFixedOffsetStyle($MColumn, fixedOffsetValue.value, level)

      return <td key={$MCellSequenceNo} className={TCellClsMixin} style={style}>{MTHB_CTT}</td>
    }
